...
<details open>
	...
	{/if}
	{#if filterLog.length}
		<pre class="log-output">{filterLog.join('\n')}</pre>
	{/if}
</details>
...
<details open>
	...
	<p>
		Reactive <code>.limit(currentPage * pageSize)</code> — collection accumulates rows, local slice
		picks current page. Same pattern as <code>channels.svelte</code>.
	</p>
	<p>
