...
const RE_MULTIPLE_WHITESPACE = /\s+/g
const RE_TRAILING_LINE_PUNCTUATION = /[.,;:!?-]*$/
...
export function buildChannelInfoCanvas(params) {
	...
	const description = String(channel.description || mediaItem.description || '')
		.replace(RE_MULTIPLE_WHITESPACE, ' ')
		.trim()
	...
	const drawWrappedText = (text, x, y, maxWidth, lineHeight, maxLines) => {
		...
		if (lines.length) {
			...
			if (consumed < words.length) {
				...
				lines[lines.length - 1] = `${last.replace(RE_TRAILING_LINE_PUNCTUATION, '')}...`
			}
		}
