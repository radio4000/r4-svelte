<script>
	...
	import {playChannelTokenInDeck, toggleChannelFavorite} from '$lib/components/channel-token-playback.js'
	...
	async function handleNavigate(href, item, kind, token) {
		if ((kind === 'tag' || kind === 'mention') && token) {
			const played = await playChannelTokenInDeck(item, kind, token, appState.active_deck_id)
			if (played) return
		}
		if (kind === 'favorite') {
			const toggled = await toggleChannelFavorite(item)
			if (toggled) return
		}
		if (onnavigate) {
