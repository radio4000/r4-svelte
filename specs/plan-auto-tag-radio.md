...
`joinAutoRadio(deckId, tracks, view?)` — derives epoch from tracks, view seed from `serializeView`, label from `viewToQuery`. Stores full `view` on deck for matching/resync. `seekToAutoRadioOffset` helper shared with `resyncAutoRadio`.
...
### 8. `api.ts` — view-based `resyncAutoRadio`

`resyncAutoRadio(deckId)` now re-filters local channel tracks using the stored deck view (`processViewTracks`) before recomputing weekly shuffle and offset. Filtered/tag/search auto-radio decks resync to their own selection.
